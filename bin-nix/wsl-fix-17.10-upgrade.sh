...
package=bash
while echo "Downloading deb file for $package" >&2 && ! apt-get download "$package"
do
    yn=
    while [ "$yn" != 'y' ]
    do
        if ! read -rn1 -p "It looks like bash didn't download successfully.  Do you want to try manually specifying a version? [y/n]: " yn
        then
            echo >&2
